...
echo -e "\e[$line•────────────────•\e[m"
echo -e ""
qrencode -t ansiutf8 "trojan://${user}@${sts}${domain}:${xtr}?sni=$sni#${user}"
echo -e ""
...
qrencode -t ansiutf8 "trojan://${user}@${sts}${domain}:443?path=$patchtls&security=tls&host=bug.com&type=ws&sni=$sni#${user}"
echo -e "\e[$line NTLS QRcode\e[m"
qrencode -t ansiutf8 "trojan://${user}@${sts}${domain}:80?host=$sni&security=none&type=ws&path=${patchnone}#${user}" 
echo ""
echo -e "Created : $harini"
echo -e "Expired : $exp"
echo ""
